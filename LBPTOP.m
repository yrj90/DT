...
%         end
%     end
    LBPXY = apply_mapping(LBPXY,mapping);
    LBPXT= apply_mapping(LBPXT,mapping);
    LBPYT = apply_mapping(LBPYT,mapping);
...
function newcodes=apply_mapping(codes,mapping)

newcodes=zeros(size(codes));
%% Apply mapping if it is defined
if isstruct(mapping)
    for i = 1:size(codes,1)
        for j = 1:size(codes,2)
            for k = 1:size(codes,3)
              newcodes(i,j,k) = mapping.table(codes(i,j,k)+1);
%               LBPXT(j,k,i) = mapping.table(LBPXT(j,k,i)+1);
%               LBPYT(i,k,j) = mapping.table(LBPYT(i,k,j)+1);
            end           
        end
